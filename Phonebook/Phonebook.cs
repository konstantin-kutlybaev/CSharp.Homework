...
public class Phonebook
{
  ...
  public void AddSubscriber(Subscriber subscriber)
  {
    ...
    public void AddNumberToSubscriber(Subscriber subscriber, PhoneNumber number)
    {
        ...
        {
            throw new ArgumentException(); // добавил исключение если один из аргументов не соотвествует значению
        }
        else
        {
        ...
        number
            ...
            UpdateSubscriber(subscriber, subscriberWithNewNumber);
        }
    }
  ...
  public void RenameSubscriber(Subscriber subscriber, string newName)
  {
    ...
    if (newName == string.Empty)
        {
            throw new ArgumentNullException("You cannot replace an employee's name with a blank one."); // добавил проверку при попытке поменять Имя на пустое значение. 
        }
  ...
  public void DeleteSubscriber(Subscriber subscriberToDelete)
{
    ...
    {
        throw new InvalidOperationException("Unable to delete subscriber. Subscriber does not exist");// добавил проверку при попытке удаление несуществующего пользователя. 
    }
}
